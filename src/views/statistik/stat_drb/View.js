...
const STAT_DRB = ({ loaded, setLoaded }) => {
    ...
    const handleChange = async () => {
        ...
        userService.getItems('/stat_drb/tampil_lap', data)
        .then((res)=> {
         if(res.status === 200)
        {
            if(res.data.tampil)
    ...
    data: [
    {
        ...
        dataPoints: items.map(item => {
            ...
            dp.y = (item.real_operasi === null ? 0 : parseInt(item.real_operasi));
            return dp;
    ...
    },
    {
        ...
        dataPoints: items.map(item => {
            ...
            var dp = {};
            dp.label = item.nm_pemda;
            dp.y = (item.real_modal === null ? 0 : parseInt(item.real_modal));
            return dp;
    ...
    {
        type: "column",
        name: "REAL_TAK_TERDUGA",
        legendText: "TAK TERDUGA",
        showInLegend: true, 
        dataPoints: items.map(item => {
            
            var dp = {};
            dp.label = item.nm_pemda;
            dp.y = (item.real_tak_terduga === null ? 0 : parseInt(item.real_tak_terduga));
            return dp;
    ...
    {
        type: "column",	
        name: "REAL_BIAYA_KELUAR",
        legendText: "BIAYA KELUAR",
        showInLegend: true,
        dataPoints: items.map(item => {
            
            var dp = {};
            dp.label = item.nm_pemda;
            dp.y = (item.real_biaya_keluar === null ? 0 : parseInt(item.real_biaya_keluar));
            return dp;
